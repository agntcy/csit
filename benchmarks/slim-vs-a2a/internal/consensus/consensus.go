...
type Finding struct {
	...
	// Payload is optional fixed-size padding used to stress transport bandwidth.
	// It carries no semantic meaning for consensus; it only inflates the wire
	// size of every finding so the relay-vs-multicast cost difference shows.
...
// only to inflate finding size for transport stress tests, so math/rand is
// intentional here: the same seed/agent yields identical bytes across both
// transports, keeping the SLIM-vs-A2A comparison fair. Not security-sensitive.
func makePayload(n int, seed int64, agentIndex int) string {
	if n <= 0 {
		return ""
	}
	const alphabet = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 "
	rng := mrand.New(mrand.NewSource(seed + int64(agentIndex)*1_000_003))
	b := make([]byte, n)
	for i := range b {
