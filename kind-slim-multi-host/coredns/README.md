...
2. [`scripts/coredns-apply-cluster-b-ingress-alias.sh`](../scripts/coredns-apply-cluster-b-ingress-alias.sh) merges a **`# BEGIN csit-cross-cluster`** … **`# END csit-cross-cluster`** block into **cluster B** only (`kube-system/coredns` `Corefile`), defining zone **`csit.test`** with **`hosts`** entries for **`control.cluster-a.csit.test`** and **`slim.cluster-a.csit.test`**, then restarts CoreDNS.

Legacy **`csit.peer`** stubs are removed; upgrades can run **`task coredns:strip:legacy-peer`** once.

In-cluster **`*.svc.cluster.local`** for the remote cluster is **not** mirrored—only these **`*.csit.test`** names for cluster **A** ingress (controller + slim dataplane gRPC).
