...
YAML `ConsensusScenario` plans for the **Agent Consensus Convergence** (v2)
consensus benchmark. Each scenario defines N agents that run a deterministic
**distributed hypothesis convergence** workload: agents think in parallel, emit findings, and
must reach identical local consensus. The goal is to show that many-to-many group multicast
beats point-to-point streaming — not to compare messaging frameworks.

## Topologies compared

The two implementations highlight one structural difference:

- **SLIM group multicast (`slim-group-session`)** — agents join a native SLIM group session and
...
- **P2P streaming (`p2p-relay-stream`)** — point-to-point streaming has no group multicast, so
  the runner is an explicit **relay hub**. Two server-streaming legs carry findings: agents stream
...
group session and P2P streaming relay subscriptions stay up for the whole run, so there is a
...
surfaces reliability differences under load: P2P streaming may miss the per-epoch budget
while SLIM still converges.

Sweeps expose two variables (defaults shown): `SWEEP_EPOCHS=10` and
`SWEEP_MAX_EPOCH_MS=30000` (a modest budget keeps failed P2P epochs capping
quickly). For example:
...
rounds) — it only inflates the wire size of each finding. Because P2P streaming relays each
...
load the P2P relay. Combine it with high agent counts and a short think window to
...
> Caveat: the streaming task store appends each relayed finding to the task
> history and deep-copies the task per update, so very large payloads also amplify
> P2P cost super-linearly. That is a property of the point-to-point streaming/task
> model, but note it when attributing the delta.
...
- `relay` (**default**) — latency is charged at the network hop. The P2P streaming relay
