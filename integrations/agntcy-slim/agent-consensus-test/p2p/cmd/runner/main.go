...
// Command runner is the P2P streaming benchmark driver and relay hub. Because
// P2P streaming has no group multicast, the runner is the explicit relay: it
// subscribes to every agent's findings stream (OpStreamFindings) and fans each
// finding out to all other agents over the relay stream (OpStreamRelay). It also
// drives start and polls snapshots to detect global consensus.
package main
...
import (
	...
	"github.com/agntcy/csit/integrations/agntcy-slim/agent-consensus-test/p2p/internal/client"
...
func main() {
	...
	agentPath := *agentBin
	if agentPath == "" {
		agentPath = os.Getenv("P2P_AGENT_BIN")
	}
	if agentPath == "" {
		log.Fatal("set --agent-bin or P2P_AGENT_BIN")
	}
	...
	for epoch := 0; epoch < s.Spec.Epochs; epoch++ {
		...
		// epoch, so streaming-task history on the findings leg does not accumulate
...
func startAgents(s *scenario.ConsensusScenario, agentBin, scenarioFile, relayCardURL string, quiet bool) []*exec.Cmd {
	...
	for i, agent := range s.Agents {
		args := []string{
			"--agent-id", agent.ID,
			"--grpc-port", fmt.Sprintf("%d", agent.P2PPort),
