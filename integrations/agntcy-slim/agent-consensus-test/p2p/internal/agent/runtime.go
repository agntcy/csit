...
// Package agent implements a consensus agent for the point-to-point (P2P)
// streaming topology.
//
// P2P streaming has no native group multicast, so every finding flows through
// the runner relay over two server-streaming legs:
...
type Runtime struct {
	...
	// when a new one starts. The streaming SDK runs executors on a detached
	// context, so a runner disconnect never cancels the server stream;
	// superseding is the only way to keep the findings-leg executions from
	// accumulating.
...
// current epoch. Each epoch gets a fresh stream so streaming-task history does
...
func (r *Runtime) StreamFindings(ctx context.Context, execCtx *a2asrv.ExecutorContext) iter.Seq2[a2a.Event, error] {
	...
	// (the streaming SDK never cancels the executor's detached context on disconnect).
	...
	for round := 0; round < spec.MaxRounds; round++ {
		...
		if emit && finding != nil {
			...
			case r.outbound <- *finding:
				benchlog.Finding(benchlog.ImplP2P, "published", r.agentIndex, finding.FindingID)
			case <-time.After(5 * time.Second):
				benchlog.Finding(benchlog.ImplP2P, "publish_timeout", r.agentIndex, finding.FindingID)
			}
			time.Sleep(emitGap)
		}
...
func (r *Runtime) applyFinding(f consensus.Finding) {
	...
	benchlog.Finding(benchlog.ImplP2P, "received", r.agentIndex, f.FindingID,
