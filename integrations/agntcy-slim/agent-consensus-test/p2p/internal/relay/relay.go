...
// Package relay implements the runner-side relay hub for the point-to-point
// (P2P) streaming topology.
//
// The hub is the central point every finding must pass through, because P2P
// streaming has no group multicast. It hosts a streaming server exposing
// OpStreamRelay: each agent subscribes and receives a server stream of every
// relayed finding. The runner separately subscribes to each agent's
// OpStreamFindings stream and calls Broadcast for every finding, which fans it
// out to all other agents' relay streams. Broadcast drops findings whose epoch
// does not match the hub's current epoch so stragglers from a prior attempt are
// not relayed. This relay fan-out is exactly the work native SLIM avoids.
package relay
...
import (
	...
	"github.com/agntcy/csit/integrations/agntcy-slim/agent-consensus-test/p2p/internal/protocol"
...
type subscriber struct {
	...
	// The streaming SDK runs executors on a detached context, so a client
	// disconnect never cancels the server-side stream; this is the only way to
	// end it.
	done     chan struct{}
	stopOnce sync.Once
}
...
func (h *Hub) Serve() error {
	handler := a2asrv.NewHandler(
		...
		a2asrv.WithTaskStore(taskstore.NewInMemory(&taskstore.InMemoryStoreConfig{
			Authenticator: func(context.Context) (string, error) { return "slim-agent-consensus", nil },
		})),
	)

	card := &a2a.AgentCard{
		Name:        "consensus-v2-relay",
		Description: "SLIM vs P2P v2 relay hub",
		Version:     "1.0.0",
